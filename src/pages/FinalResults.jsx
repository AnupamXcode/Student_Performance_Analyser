...
import { Trophy, FileText, Download, CheckCircle2 } from 'lucide-react';
      ...
      <div className="glass-panel p-6 space-y-4 border-l-4 border-emerald-500">
        ...
        <div className="space-y-3 text-xs text-slate-700 dark:text-slate-300 font-normal leading-relaxed">
          <p>
            ...
            The distribution shape is classified as <strong>{meanMed?.DistributionShape}</strong> with a skewness score of <code>{meanMed?.Skewness}</code>. 
            {meanMed?.RecommendedEstimator}.
          </p>

          <p>
            <strong>2. Sampling Accuracy:</strong> Evaluating sampling performance at $N=50$, <strong>{sampling?.bestMethod}</strong> provided the most accurate estimate of the population mean 
            with a minimum deviation of <code>{sampling?.comparisonTable.find(m => m.Method === sampling.bestMethod)?.DiffFromPopMean}</code>.
          </p>
